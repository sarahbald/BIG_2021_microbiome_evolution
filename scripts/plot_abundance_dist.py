...
species_abun_dict_zeros = calculate_species_relative_abundance.load_species_abun_dict()
species_abun_dict = {}
...
species_prev_dict = calculate_species_relative_abundance.load_species_prev_dict()
africa_prev = []
na_prev = []
species_order_prev = []
...
(mu,sigma) = Klogn(np.array(na_mean_abun), c)
axs[0,0].plot(np.exp(x), np.sqrt(2/math.pi)/sigma *np.exp(-(x-mu)**2 /2/(sigma**2))/scipy.special.erfc((np.log(c)-mu)/np.sqrt(2)/sigma))
(mu,sigma) = Klogn(np.array(africa_mean_abun), c)
axs[0,0].plot(np.exp(x), np.sqrt(2/math.pi)/sigma *np.exp(-(x-mu)**2 /2/(sigma**2))/scipy.special.erfc((np.log(c)-mu)/np.sqrt(2)/sigma))
...
axs[1, 1].legend()
plt.savefig('C:/Users/sarah/Garud Lab/plots/macroecology_plot.png', bbox_inches='tight', dpi=600)
