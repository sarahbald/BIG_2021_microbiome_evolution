...
def calculate_consensus_prevalence(allele_counts_matrix,lower_threshold=0,upper_threshold=1,depths_min=10):

    num_sites, num_samples, num_alleles = allele_counts_matrix.shape

    depths = allele_counts_matrix.sum(axis=2)
    freqs = allele_counts_matrix[:,:,0]*1.0/(depths+(depths==0))
    passed_sites_matrix = (depths>depths_min)
    # consensus approximation
    #genotype_matrix = numpy.around(freqs)*passed_sites_matrix
    #prevalence_matrix = numpy.logical_or(freqs<=lower_threshold,freqs>=upper_threshold)

    prevalence_matrix = passed_sites_matrix*numpy.where((freqs > lower_threshold) & (freqs < upper_threshold), 1, 0)

    return prevalence_matrix, passed_sites_matrix
...
def calculate_unbiased_sigmasquared(allele_counts_1, allele_counts_2):
    ...
    ratio = rsquared_numerators/rsquared_denominators
...
def calculate_prevalence_matrix(allele_counts_map, passed_sites_map, allowed_sample_idxs=[], allowed_variant_types = set(['1D','2D','3D','4D']), allowed_genes=set([]),pi_min_k=1,lower_threshold=0.2,upper_threshold=0.8):
...
def calculate_mean_pi_matrix_per_pathway(pi_per_gene, avg_pi_per_gene, passed_sites_per_gene,num_people_with_data, kegg_ids,min_passed_sites_per_person=100):
    ...
    for pathway_name in avg_pi_per_pathway.keys():
        ...
        #print(num_genes_per_pathway[pathway_name])
        if num_genes_per_pathway[pathway_name] ==0:
            continue
...
def calculate_mean_fixation_matrix_per_pathway(fixation_per_gene, passed_sites_per_gene,num_people_with_data, kegg_ids, min_passed_sites_per_person=100):
    ...
    for pathway_name in fixation_per_pathway.keys():
       # we want to identify people that have few passed sites even after aggregating the data accross genes. Then set the values in these cells to zero because these data points are too noisy
        ...
        if float(num_genes_per_pathway[pathway_name]) == 0:
            continue
...
############################################################################
# functions from Alberts code
############################################################################

def calculate_conditional_unbiased_sigmasquared(allele_counts_1, allele_counts_2, low_freq=0.0, high_freq=1):
		# An alternate version of a standard measure of linkage disequilibrium:
		#
		# sigma_squared= E[X]/E[Y], where X=(p_ab-pa*pb)^2 and Y=(pa*(1-pa)*pb*(1-pb))
		# rsquared=E[X/Y]
		...
		#if condition_on_freq:
        if (low_freq > 0.0) or (high_freq < 1):
            depths_1 = allele_counts_1.sum(axis=2)
            freqs_1 = allele_counts_1[:, :, 0] * 1.0 / (depths_1 + (depths_1 == 0))
            depths_2 = allele_counts_2.sum(axis=2)
            freqs_2 = allele_counts_2[:, :, 0] * 1.0 / (depths_2 + (depths_2 == 0))
            ...
            # calculate frequencies from read counts
            pooled_freqs_1 = freqs_1[:, :].sum(axis=1) / len(freqs_1[0])
            pooled_freqs_2 = freqs_2[:, :].sum(axis=1) / len(freqs_2[0])

			# check if any freqs >0.5, if so, fold:
			# np where is a ternary operator if then else
            # fold the site frequency spectrum since we don't know the "true" reference allele
            # frequency range fmin = 0, fmax = 0.5
            pooled_freqs_1 = numpy.where(pooled_freqs_1 > 0.5, 1 - pooled_freqs_1, pooled_freqs_1)
            pooled_freqs_2 = numpy.where(pooled_freqs_2 > 0.5, 1 - pooled_freqs_2, pooled_freqs_2)

            passed_freq_sites_1 = (depths_1 > 0) * (pooled_freqs_1 >= low_freq)[:, None] * (pooled_freqs_1 <= high_freq)[:, None]
            passed_freq_sites_2 = (depths_2 > 0) * (pooled_freqs_2 >= low_freq)[:, None] * (pooled_freqs_2 <= high_freq)[:, None]

            joint_passed_sites = passed_freq_sites_1[None, :, :] * passed_freq_sites_2[:, None, :]
...
#####################################################################
def calculate_unbiased_sigma_k_vector(allele_counts_1, allele_counts_2, f0, k=1):

    # Calculate the kth moment of sigma using equations in Good 2020
    #
    # where we have corrected for finite sample effects

    # accepts arguments of k=1,2
    ...
    # allele counts
    ns = len(allele_counts_1)


    n11s = sum(numpy.logical_and(allele_counts_1, allele_counts_2))
    n10s = sum(allele_counts_1[allele_counts_2 == 0])
    n01s = sum(allele_counts_2[allele_counts_1 == 0])
    n00s = ns - n11s - n10s - n01s


    # work with log due to overflow

    def M_function(n_tuple, ns_, i, j, k, l, f0):
        n11s_, n10s_, n01s_, n00s_ = n_tuple

        #m_11_numer = math.factorial(n11s_)* ((1-(2/(ns_*f0)))**(n11s_ - k))
        #if n11s_-k < 0:
        #    m_11_denom = 0
        #else:
        #    m_11_denom = (ns_**k)*math.factorial(n11s_-k)

        #m_10_numer = math.factorial(n10s_)* ((1-(1/(ns_*f0)))**(n10s_-i))
        #if n10s_-i < 0:
        #    m_10_denom = 0
        #else:
        #    m_10_denom = (ns_**i)*math.factorial(n10s_-i)

        #m_01_numer_log = math.log(math.factorial(n01s_)) + math.log(((1-(1/(ns_*f0)))**(n01s_-j)))
        #m_01_denom_log = j*math.log(ns_) + math.log(n01s_-j)

        #m_01_numer = math.factorial(n01s_)* ((1-(1/(ns_*f0)))**(n01s_-j))
        #if n01s_-j < 0:
        #    m_01_denom = 0
        #else:
        #    m_01_denom = (ns_**j)*math.factorial(n01s_-j)

        #math.factorial(n01s_)* ((1-(1/(ns_*f0)))**(n01s_-j))

        #m_00_numer = math.factorial(n00s_)
        #if n00s_-l < 0:
        #    m_00_denom = 0
        #else:
        #    m_00_denom = (ns_**l)*math.factorial(n00s_-l)





        if (n11s_-k < 0) or (1-(2/(ns_*f0)) < 0):
            m_11_numer_log = float("nan")
            m_11_denom_log = float("nan")
        else:
            m_11_numer_log = math.log(math.factorial(n11s_)) + math.log(((1-(2/(ns_*f0)))**(n11s_ - k)))
            m_11_denom_log = k*math.log(ns_) + math.log(math.factorial(n11s_-k))



        if (n10s_-i < 0) or (1-(2/(ns_*f0)) < 0):
            m_10_numer_log = float("nan")
            m_10_denom_log = float("nan")
        else:
            m_10_numer_log = math.log(math.factorial(n10s_)) + math.log(((1-(2/(ns_*f0)))**(n10s_ - i)))
            m_10_denom_log = i*math.log(ns_) + math.log(math.factorial(n10s_-i))



        if (n01s_-j < 0) or (1-(2/(ns_*f0)) < 0):
            m_01_numer_log = float("nan")
            m_01_denom_log = float("nan")
        else:
            m_01_numer_log = math.log(math.factorial(n01s_)) + math.log(((1-(2/(ns_*f0)))**(n01s_ - j)))
            m_01_denom_log = j*math.log(ns_) + math.log( math.factorial(n01s_-j))



        if (n00s_-l < 0) or (1-(2/(ns_*f0)) < 0):
            m_00_numer_log = float("nan")
            m_00_denom_log = float("nan")
        else:
            m_00_numer_log = math.log(math.factorial(n00s_))
            m_00_denom_log = l*math.log(ns_) + math.log(math.factorial(n00s_-l))



        if float("nan") in [m_11_denom_log, m_10_denom_log, m_01_denom_log, m_00_denom_log]:
            return 0

        else:

            m_11_log = m_11_numer_log - m_11_denom_log
            m_10_log = m_10_numer_log - m_10_denom_log
            m_01_log = m_01_numer_log - m_01_denom_log
            m_00_log = m_00_numer_log - m_00_denom_log

            return math.exp(m_11_log+m_10_log+m_01_log+m_00_log)


    sigma_k_denominator = M_function((n11s, n10s, n01s, n00s), ns, 2, 2, 0, 0, f0)

    sigma_k_denominator += M_function((n11s, n10s, n01s, n00s), ns, 1, 2, 0, 1, f0)
    sigma_k_denominator += M_function((n11s, n10s, n01s, n00s), ns, 2, 1, 1, 0, f0)
    sigma_k_denominator += 2*M_function((n11s, n10s, n01s, n00s), ns, 1, 1, 1, 1, f0)
    sigma_k_denominator += M_function((n11s, n10s, n01s, n00s), ns, 2, 1, 0, 1, f0)
    sigma_k_denominator += M_function((n11s, n10s, n01s, n00s), ns, 1, 1, 0, 2, f0)
    sigma_k_denominator += M_function((n11s, n10s, n01s, n00s), ns, 2, 0, 1, 1, f0)
    sigma_k_denominator += 2*M_function((n11s, n10s, n01s, n00s), ns, 1, 0, 1, 2, f0)
    sigma_k_denominator += M_function((n11s, n10s, n01s, n00s), ns, 1, 2, 1, 0, f0)
    sigma_k_denominator += M_function((n11s, n10s, n01s, n00s), ns, 0, 2, 1, 1, f0)
    sigma_k_denominator += M_function((n11s, n10s, n01s, n00s), ns, 1, 1, 2, 0, f0)
    sigma_k_denominator += M_function((n11s, n10s, n01s, n00s), ns, 0, 1, 2, 1, f0)
    sigma_k_denominator += M_function((n11s, n10s, n01s, n00s), ns, 0, 1, 1, 2, f0)
    sigma_k_denominator += M_function((n11s, n10s, n01s, n00s), ns, 0, 0, 2, 2, f0)

    if k == 1:
        sigma_k_numerator = M_function((n11s, n10s, n01s, n00s), ns, 0, 0, 1, 1, f0)
        sigma_k_numerator -= M_function((n11s, n10s, n01s, n00s), ns, 1, 1, 0, 0, f0)

    else:
        sigma_k_numerator = M_function((n11s, n10s, n01s, n00s), ns, 0, 0, 2, 2, f0)
        sigma_k_numerator += M_function((n11s, n10s, n01s, n00s), ns, 2, 2, 0, 0, f0)
        sigma_k_numerator -= 2*M_function((n11s, n10s, n01s, n00s), ns, 1, 1, 1, 1, f0)

    #to_keep = numpy.logical_not(numpy.isnan(sigma_k_numerator))

    #sigma_k_numerator = sigma_k_numerator[to_keep]
    #sigma_k_denominator = sigma_k_denominator[to_keep]

    return sigma_k_numerator, sigma_k_denominator
...
#####################################################################
def calculate_unbiased_sigma_k(allele_counts_1, allele_counts_2, f0, k=1):
    # Calculate the kth moment of sigma using equations in Good 2020
    #
    # where we have corrected for finite sample effects
    ...
    def M_function(n_tuple, ns_, i, j, k, l, f0):
        ...
        m_11_numer = factorial(n11s_)* ((1-(2/(ns_*f0)))**(n11s_ - k))
        m_11_denom = (ns_**k)*factorial(n11s_-k)

        m_10_numer = factorial(n10s_)* ((1-(1/(ns_*f0)))**(n10s_-i))
        m_10_denom = (ns_**i)*factorial(n10s_-i)

        m_01_numer = factorial(n01s_)* ((1-(1/(ns_*f0)))**(n01s_-j))
        m_01_denom = (ns_**j)*factorial(n01s_-j)

        m_00_numer = factorial(n00s_)
        m_00_denom = (ns_**l)*factorial(n00s_-l)

        m_11 = m_11_numer/m_11_denom
        m_10 = m_10_numer/m_10_denom
        m_01 = m_01_numer/m_01_denom
        m_00 = m_00_numer/m_00_denom
...
#####################################################################
def calculate_unbiased_sigmasquared_prevalence(allele_counts_1, allele_counts_2, coverage_threshold = 10):
    # An alternate version of a standard measure of linkage disequilibrium:
    #
    # sigma_squared= E[X]/E[Y], where X=(p_ab-pa*pb)^2 and Y=(pa*(1-pa)*pb*(1-pb))
    # rsquared=E[X/Y]
    ...
    genotypes_1, passed_sites_1 = calculate_consensus_prevalence(allele_counts_1)
    genotypes_2, passed_sites_2 = calculate_consensus_prevalence(allele_counts_2)
...
#####################################################################
def calculate_unbiased_sigma_k_prevalence(allele_counts_1, allele_counts_2, f0, k=1):
    # Calculate the kth moment of sigma using equations in Good 2020
    #
    # where we have corrected for finite sample effects
...
def calculate_unbiased_sigma_k_prevalence_mapgd(allele_counts_1, allele_counts_2, f0, k=1):
    # Calculate the kth moment of sigma using equations in Good 2020
    #
    # where we have corrected for finite sample effects
    ...
    allele_counts_1 = allele_counts_1[0]
    allele_counts_2 = allele_counts_2[0]
    ...
    n11s = sum(numpy.logical_and(allele_counts_1>0, allele_counts_2>0))
    n10s = sum(numpy.logical_and(allele_counts_1>0, allele_counts_2==0))
    n01s = sum(numpy.logical_and(allele_counts_1==0, allele_counts_2>0))
    n00s = sum(numpy.logical_and(allele_counts_1==0, allele_counts_2==0))
